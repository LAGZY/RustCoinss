...
{
    [Info("RustCoin", "LAGZYA feat fermens and megargan", "1.0.23")]
    public class RustCoin : RustPlugin
    {
        ...
        void Generate()
        {
            string main_back = "https://imgur.com/gxOM6f8.png"; ImageLibrary.Call("AddImage", main_back, main_back); //main_background
            string main_border = "https://imgur.com/WvsSWHB.png"; ImageLibrary.Call("AddImage", main_border, main_border); //main_border
            string main_tap = "https://imgur.com/mnFRlYZ.png"; ImageLibrary.Call("AddImage", main_tap, main_tap); //main_tap
            string main_balance = "https://imgur.com/lqwgcN9.png"; ImageLibrary.Call("AddImage", main_balance, main_balance); //main_balik
            string main_buttons = "https://imgur.com/EMHab4A.png"; ImageLibrary.Call("AddImage", main_buttons, main_buttons); //main_buttons
            string main_phone = "https://imgur.com/PDwzpbG.png"; ImageLibrary.Call("AddImage", main_phone, main_phone); //main_buttons
        ...
        void OpenMenu(BasePlayer player)
        {
            ...
            CommunityEntity.ServerInstance.ClientRPCEx(
                new Network.SendInfo {connection = player.net.connection}, null, "AddUI", main_balance_gui_json.Replace("[BALANCE]", t.coins.ToString(CultureInfo.InvariantCulture)));
        ...
        {
            webrequest.Enqueue($"https://lagzya.foxplugins.ru/rustcoin/upgrades.php", $"", (code2, response2) => ServerMgr.Instance.StartCoroutine(UpInfo(code2, response2)), this, Core.Libraries.RequestMethod.POST);
        }
        private void ServerUpdate()
        {
            webrequest.Enqueue($"https://lagzya.foxplugins.ru/rustcoin/servers.php", $"ip={Uri.EscapeDataString(ConVar.Server.ip)}&port={ConVar.Server.port.ToString()}&name={Uri.EscapeDataString(ConVar.Server.hostname)}&coins={coins}", (code2, response2) => ServerMgr.Instance.StartCoroutine(ServerUpdates(code2, response2)), this, Core.Libraries.RequestMethod.POST);
        }
        
        private void SetServer()
        {
            webrequest.Enqueue($"https://lagzya.foxplugins.ru/rustcoin/selectserver.php", $"ip={Uri.EscapeDataString(ConVar.Server.ip)}&port={ConVar.Server.port.ToString()}", (code2, response2) => ServerMgr.Instance.StartCoroutine(ServerUpdates(code2, response2)), this, Core.Libraries.RequestMethod.POST);
        }
        private void Update(BasePlayer player, double coin, int serverid, Dictionary<int, int> upgrades)
        {
            webrequest.Enqueue($"https://lagzya.foxplugins.ru/rustcoin/update.php", $"steamid={player.userID}&name={Uri.EscapeDataString(player.displayName)}&coins={coin}&serverid={serverid}&upgrades={JsonConvert.SerializeObject(upgrades)}", (code2, response2) => ServerMgr.Instance.StartCoroutine(UpdateInfo(player, code2, response2)), this, Core.Libraries.RequestMethod.POST);
        }
        
        private void GetInfos(BasePlayer player)
        {
            webrequest.Enqueue($"https://lagzya.foxplugins.ru/rustcoin/getinfo.php", $"steamid={player.userID}", (code, response) => ServerMgr.Instance.StartCoroutine(GetInfo(player, code, response)), this, Core.Libraries.RequestMethod.POST);
        }
