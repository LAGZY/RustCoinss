...
namespace Oxide.Plugins
{
    ...
    public class RustCoin : RustPlugin
    {
        ...
        void OpenMenu(BasePlayer player)
        {
            ...
            var text = main_json.Replace("[MAIN_AVATAR]",
                    avatar).Replace("[NICKNAME]", player.displayName).Replace("[ID]", t.id.ToString())
                .Replace("[BALANCE]", t.coins.ToString(CultureInfo.InvariantCulture)).Replace("[TOP_POSITION]", "1");
            CommunityEntity.ServerInstance.ClientRPCEx(
                new Network.SendInfo {connection = player.net.connection}, null, "AddUI", text);
        }
